...
namespace bczhc {
    ...
    public:
        ...
        void sort(T *arr, int length, Comparable<T> &comparable) {
            this->c = &comparable;
    ...
    private:
        ...
        int partition(T *arr, int left, int right) {
            ...
            for (;;) {
                while (c->compare(arr[--q], pivot) > 0 && q != left) {
                }
                while (c->compare(arr[++p], pivot) <= 0 && p != right) {
                }
                if (q <= p)
                    break;
                else
